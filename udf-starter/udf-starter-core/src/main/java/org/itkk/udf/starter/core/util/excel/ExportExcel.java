 ...
 */
package org.itkk.udf.starter.core.util.excel;
...
public class ExportExcel {
    ...
    public void write(HttpServletRequest request, HttpServletResponse response, String fileName) throws IOException {
        ...
        try {
            response.reset();
            response.setContentType(CoreConstant.CONTENT_TYPE_XLSX);
            ...
            if (request.getHeader("User-Agent").toUpperCase().indexOf("MSIE") > 0) {
                response.setHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(fileName, CoreConstant.CHARACTER_SET));
            } else {
                response.setHeader("Content-Disposition", "attachment;filename*=utf-8'zh_cn'" + URLEncoder.encode(fileName, CoreConstant.CHARACTER_SET));
            }
